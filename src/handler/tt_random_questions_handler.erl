...
%%% @doc  cowboy random questions handler
%%% @end
%%% Created : 10. Nov 2015 2:27 AM
%%%-------------------------------------------------------------------
-module(tt_random_questions_handler).
-author("Felipe Ripoll <ferigis@gmail.com>").
...
%% @hidden
forbidden(Req, State) ->
  {Number, _} = cowboy_req:binding(number, Req),
  case Number of
    undefined ->
      {true, Req, State#{val => null}};
    Number ->
      {false, Req, State#{val => total_testing:get_random_questions(binary_to_integer(Number))}}
  end.
